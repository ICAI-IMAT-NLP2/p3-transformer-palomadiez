...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size: int, tgt_vocab_size: int, max_enc_position_embeddings: int, max_dec_position_embeddings: int,
        ...
        self.encoder = TransformerEncoder(
                src_vocab_size, max_enc_position_embeddings,
                enc_d_model, num_attention_heads,
                enc_intermediate_size, num_enc_hidden_layers
            )        
        self.decoder = TransformerDecoder(
                tgt_vocab_size, max_dec_position_embeddings,
                dec_d_model, num_attention_heads,
                dec_intermediate_size, num_dec_hidden_layers
            )
    ...
    def forward(self, src_input: torch.Tensor, tgt_input: torch.Tensor, attn_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        ec_output = self.decoder(tgt_input, enc_output)

        # Project the decoder output to the target vocabulary size
        dec_output = self.output_linear(ec_output)
    ...
    def __greedy_decode(self, src_input: torch.Tensor, max_length: int, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            logits = self.output_linear(dec_output) # Shape: (batch_size, vocab_size)
            last_logits = logits[:, -1, :]
            # Get the token with the highest probability
            next_token = last_logits.argmax(dim=-1, keepdim=True) # Shape: (batch_size, 1)
    ...
    def __beam_search_decode(self, src_input: torch.Tensor, max_length: int, beam_size: int = 3, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                dec_output = self.decoder(tgt_input, enc_output)
                # Get the logits for the last time step
                logits = self.output_linear(dec_output)  # Shape: (1, vocab_size)
                last_logits = logits[:, -1, :]
                # Apply log softmax to get log probabilities
                log_probs = torch.log_softmax(last_logits, dim=-1)  # Shape: (1, vocab_size)
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            last_logits = logits[:,-1,:]
            # Apply log softmax to get log probabilities
            probs = torch.log_softmax(last_logits, dim=-1)
            # Get the top k tokens
            topk_probs, topk_indices = torch.topk(probs, k=k, dim=-1)

            # Prepare tensors for all candidates
            expanded_tgt_input = tgt_input.unsqueeze(1).expand(-1, k, -1).reshape(batch_size * k, -1)  # Shape: (k, seq_len)
            next_tokens = topk_indices.reshape(-1,1)  # Shape: (k, 1)
            ...
            # Compute scores
            P_LM_v = topk_probs.reshape(-1)  # Shape: (k,)
            scores = P_LM_v - alpha * max_sim  # Shape: (k,)

            # Select the candidate with the highest score
            scores = scores.view(batch_size, k)
            best_idx = scores.argmax(dim=-1)
            best_token = topk_indices[torch.arange(batch_size), best_idx].unsqueeze(1)  # Shape: (1, 1)
